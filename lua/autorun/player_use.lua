...
if SERVER then
	...
	hook.Add("PlayerUse", Tag, function(initator, target)
		...
		if not initator:IsPlayer() or not initator:KeyPressed(IN_USE) then return end
...
local function getStartledBy(target, me)
	...
	hook.Add("StartCommand", Tag, function(pl, mv)
		...
		if not me:Alive() or me:GetMoveType() ~= MOVETYPE_WALK or not me:IsOnGround() then return end --(SERVER and not me:IsBot()) or
